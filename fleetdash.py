...
# Define the Fleet API endpoint used to gather the hosts telemetry (https://fleetdm.com/docs/using-fleet/rest-api#hosts)
# url = "https://fleetserver/api/v1/fleet/hosts"
headers = {
    #'Authorization': 'Bearer <token>'
}

# Use requests library to make a GET call to the hosts endpoint
response = requests.request("GET", url, headers=headers)

# Normalize the response to only include the hosts object
hosts = response.json()
host_data = json_normalize(hosts['hosts'])

# Data Handling using Pandas library
...
# Define the html layout via Dash components
...
# Create the app callback to re-fetch data if the host status combo box is updated
@app.callback(
